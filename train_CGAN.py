...
# training
parser.add_argument('--epoch', dest='epoch', type=int, default=50)#训练次数
parser.add_argument('--batch_size', dest='batch_size', type=int, default=64)#批次大小
parser.add_argument('--d_learning_rate', dest='d_learning_rate', type=float, default=0.0002)#判别器学习率
...
for ep in range(start_ep, epoch):
    for i, (x, c_dense) in enumerate(train_loader):
        step = ep * len(train_loader) + i + 1#按每一个批次进行绘图
        D.train()
        G.train()

        # train D
        x = x.to(device)
        z = torch.randn(batch_size, z_dim).to(device)#生成噪声
        c = torch.tensor(np.eye(c_dim)[c_dense.cpu().numpy()], dtype=z.dtype).to(device)#对编码进行量纲统一
